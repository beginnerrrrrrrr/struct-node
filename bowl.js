...
// ── HiDPI / zoom fix ──────────────────────────────────────────────────
// Multiply physical pixels by the device ratio, keep CSS size at 90×90.
// This prevents pixelation at 150 %, 200 % zoom or on Retina screens.
const dpr = window.devicePixelRatio || 1;
bCanvas.width        = BOWL_W * dpr;
bCanvas.height       = BOWL_H * dpr;
bCanvas.style.width  = BOWL_W + 'px';
bCanvas.style.height = BOWL_H + 'px';
bCtx.scale(dpr, dpr);

// Shared state — app.js sets bowlTargetPct, bowl animates toward it
let bowlTargetPct  = 0;
...
function getWaterBounds(pct) {
  ...
  const waterLevel = fillBot - (fillBot - fillTop) * (pct / 100);
...
function drawBowlFrame() {
  ...
  if (bowlCurrentPct > 0.3) {
    ...
    bCtx.strokeStyle = 'rgba(180,240,255,0.3)'; bCtx.lineWidth = 1.5; bCtx.stroke();
    ...
    for (let i = 0; i < 4; i++) {
      ...
      bCtx.beginPath(); bCtx.arc(bx, by, 1.2, 0, Math.PI * 2);
      bCtx.fillStyle = 'rgba(200,240,255,0.22)'; bCtx.fill();
    }
    ...
    bCtx.fillStyle = cl;
    bCtx.beginPath(); bCtx.ellipse(26, waterLevel + 8, 11, 4, 0.4, 0, Math.PI * 2); bCtx.fill();
  }
  bCtx.restore();
  ...
  bCtx.save();
  drawBowlShape(bCtx);
  bCtx.fillStyle   = 'rgba(180,220,255,0.025)'; bCtx.fill();
  bCtx.strokeStyle = 'rgba(255,255,255,0.18)';  bCtx.lineWidth = 1.4; bCtx.stroke();
  bCtx.beginPath(); bCtx.moveTo(18, 20); bCtx.bezierCurveTo(12, 36, 15, 43, 15, 50);
  bCtx.strokeStyle = 'rgba(255,255,255,0.14)'; bCtx.lineWidth = 2.5; bCtx.lineJoin = 'round'; bCtx.stroke();
  bCtx.beginPath(); bCtx.moveTo(34, 6); bCtx.bezierCurveTo(45, 2, 54, 2, 58, 6);
  bCtx.strokeStyle = 'rgba(255,255,255,0.28)'; bCtx.lineWidth = 1.2; bCtx.stroke();
  bCtx.restore();
  ...
  if (bowlCurrentPct > 3) {
    ...
    bCtx.save();
    bCtx.font = 'bold 11px "Syne",sans-serif'; bCtx.textAlign = 'center'; bCtx.textBaseline = 'middle';
    bCtx.shadowColor = 'rgba(0,0,0,0.6)'; bCtx.shadowBlur = 4;
  ...
  // Advance ripples
  ripples.forEach(rp => { rp.r += (rp.maxR - rp.r) * rp.speed * 0.12; rp.alpha *= 0.955; });
